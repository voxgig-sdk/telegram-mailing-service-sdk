...
func TestMailingDirect(t *testing.T) {
	t.Run("direct-list-mailing", func(t *testing.T) {
		setup := mailingDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-mailing", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-mailing", func(t *testing.T) {
		setup := mailingDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-mailing", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type mailingDirectSetupResult struct {
	client *sdk.TelegramMailingServiceSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func mailingDirectSetup(mockres any) *mailingDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &mailingDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &mailingDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
