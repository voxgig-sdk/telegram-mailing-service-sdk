...
import (
	...
	omni "github.com/voxgig-sdk/telegram-mailing-service-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TelegramMailingServiceError); is {
				out := map[string]any{
					"name":    "TelegramMailingServiceError",
...
func omniCtx(arg any, client *sdk.TelegramMailingServiceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TelegramMailingServiceSDK); is {
			client = live
