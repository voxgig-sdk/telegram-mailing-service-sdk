...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TELEGRAMMAILINGSERVICE_TEST_LIVE") == "TRUE" ||
		os.Getenv("TELEGRAMMAILINGSERVICE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TELEGRAMMAILINGSERVICE_TEST_EXPLAIN"); explain != "" {
