...
[MCP](https://modelcontextprotocol.io) server exposing the TelegramMailingService SDK as
two agent tools — `telegram-mailing-service_list` and `telegram-mailing-service_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/telegram-mailing-service-mcp)
make build

# 2. Provide credentials via the environment
export TELEGRAM_MAILING_SERVICE_APIKEY=sk_live_xxx
...
```jsonc
// telegram-mailing-service_list: first page of records
{ "entity": "mailing" }
{ "entity": "mailing", "query": { } }

// telegram-mailing-service_load: one record by id
{ "entity": "mailing", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/telegram-mailing-service-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user telegram-mailing-service \
     -- "$PWD"/dist/*/telegram-mailing-service-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `telegram-mailing-service_list` and `telegram-mailing-service_load` tools now appear
   in new sessions. Ask the agent to *"list mailing using telegram-mailing-service"*
   and it calls `telegram-mailing-service_list` with `{"entity":"mailing"}`.
...
```sh
export TELEGRAM_MAILING_SERVICE_APIKEY=sk_live_xxx            # API key
export TELEGRAM_MAILING_SERVICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `telegram-mailing-service_list` tool
...
### Call the `telegram-mailing-service_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `telegram-mailing-service_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `telegram-mailing-service_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TELEGRAM_MAILING_SERVICE_APIKEY` | API key sent with every request. |
| `TELEGRAM_MAILING_SERVICE_BASE` | Optional override of the API base URL. |
