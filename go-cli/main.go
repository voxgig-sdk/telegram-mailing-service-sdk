// Command TelegramMailingService-cli is a CLI + REPL for the TelegramMailingService SDK,
...
const prompt = "telegram-mailing-service"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "mailing"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TELEGRAM_MAILING_SERVICE_APIKEY (api key), TELEGRAM_MAILING_SERVICE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TELEGRAM_MAILING_SERVICE_APIKEY carries the API key and
	// TELEGRAM_MAILING_SERVICE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TELEGRAM_MAILING_SERVICE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TELEGRAM_MAILING_SERVICE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTelegramMailingServiceSDK(opts)
