...
boru-driven command-line client **and** interactive REPL for the TelegramMailingService
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/telegram-mailing-service-cli)
make build

# 2. See usage (words, entities, env vars)
./telegram-mailing-service-cli --help
...
./telegram-mailing-service-cli load 1 mailing            # {id:1} shorthand
./telegram-mailing-service-cli load '{id:1}' mailing       # explicit match map

# 5. Override the API base URL for a single call
TELEGRAM_MAILING_SERVICE_BASE=https://api.example.com ./telegram-mailing-service-cli list mailing

# 6. No arguments -> interactive REPL
./telegram-mailing-service-cli
telegram-mailing-service> list mailing
telegram-mailing-service> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/telegram-mailing-service-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/telegram-mailing-service-cli list mailing
   ```
...
```sh
./telegram-mailing-service-cli load 1 mailing          # scalar shorthand for {id:1}
./telegram-mailing-service-cli load '{id:1}' mailing     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `telegram-mailing-service>`). Each line is
evaluated as its own boru expression:

```text
$ ./telegram-mailing-service-cli
telegram-mailing-service> list mailing
telegram-mailing-service> /help
telegram-mailing-service> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `mailing`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/telegram-mailing-service-cli`. |
...
(`./telegram-mailing-service-cli <expr>`) and interactively (the REPL), and expressions compose the
