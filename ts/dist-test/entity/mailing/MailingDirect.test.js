...
(0, node_test_1.describe)('MailingDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.TelegramMailingServiceSDK({
    ...
    (0, node_test_1.test)('direct-load-mailing', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-mailing', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-mailing', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-mailing', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.TELEGRAM_MAILING_SERVICE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.TelegramMailingServiceSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.TELEGRAM_MAILING_SERVICE_APIKEY,
        }));
        let idmap = env['TELEGRAM_MAILING_SERVICE_TEST_MAILING_ENTID'];
    ...
    const client = new __1.TelegramMailingServiceSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=MailingDirect.test.js.map