...
(0, node_test_1.describe)('MailingEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when TELEGRAM_MAILING_SERVICE_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('TELEGRAM_MAILING_SERVICE_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.TelegramMailingServiceSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        (0, node_assert_1.default)(null != mailing_ref01_data.id);
        // LIST
        const mailing_ref01_match = {};
        const mailing_ref01_list = (await mailing_ref01_ent.list(mailing_ref01_match)).map((e) => e.data());
        (0, node_assert_1.default)(!isempty(select(mailing_ref01_list, { id: mailing_ref01_data.id })));
        // LOAD
        const mailing_ref01_match_dt0 = {};
        ...
        (0, node_assert_1.default)(mailing_ref01_data_dt0.id === mailing_ref01_data.id);
        // REMOVE
        const mailing_ref01_match_rm0 = { id: mailing_ref01_data.id };
        await mailing_ref01_ent.remove(mailing_ref01_match_rm0);
        // LIST
        const mailing_ref01_match_rt0 = {};
        const mailing_ref01_list_rt0 = (await mailing_ref01_ent.list(mailing_ref01_match_rt0)).map((e) => e.data());
        (0, node_assert_1.default)(isempty(select(mailing_ref01_list_rt0, { id: mailing_ref01_data.id })));
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/mailing/MailingTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.TelegramMailingServiceSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['mailing01', 'mailing02', 'mailing03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        ...
        client = new __1.TelegramMailingServiceSDK(merge([
...
//# sourceMappingURL=MailingEntity.test.js.map