# TelegramMailingService TypeScript SDK Reference

Complete API reference for the TelegramMailingService TypeScript SDK.
...
```ts
new TelegramMailingServiceSDK(options?: object)
```
...
#### `TelegramMailingServiceSDK.test(testopts?, sdkopts?)`
...
**Returns:** `TelegramMailingServiceSDK` instance in test mode.
...
Create a new `Mailing` entity instance.
...
**Returns:** `MailingEntity` instance.
...
Alias for `TelegramMailingServiceSDK.test()`.
...
| `failedCount` | `number` | No | Number of messages that failed to send |
...
| `sentCount` | `number` | No | Number of messages successfully sent |
| `status` | `string` | No | Current status of the mailing |
| `totalRecipients` | `number` | No | Total number of recipients |
...
Return the parent `TelegramMailingServiceSDK` instance.
