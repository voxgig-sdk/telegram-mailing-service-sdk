...
//     (\`new TelegramMailingServiceSDK(...)\` / \`TelegramMailingServiceSDK.test(...)\` become
//     \`TelegramMailingServiceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TelegramMailingServiceSDK'
...
const TEST_SEED = {"entity":{"mailing":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TelegramMailingServiceSDK(...)\` and any
// \`TelegramMailingServiceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TelegramMailingServiceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TelegramMailingServiceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
