...
describe("MailingDirect", function()
  it("should direct-list-mailing", function()
    local setup = mailing_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-mailing", setup.live and "live" or "unit")
  ...
  it("should direct-load-mailing", function()
    local setup = mailing_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-mailing", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function mailing_direct_setup(mockres)
