...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Mailing()` — each with the same small set of operations (`list`, `load`, `create`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── telegram-mailing-service_sdk.lua    -- Main SDK module
...
The main module (`telegram-mailing-service_sdk`) exports the SDK constructor
