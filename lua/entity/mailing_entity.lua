...
local MailingEntity = {}
MailingEntity.__index = MailingEntity


function MailingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MailingEntity)
  self._name = "mailing"
  self._client = client
...
function MailingEntity:get_name()
  return self._name
end


function MailingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MailingEntity.new(self._client, opts)
end
...
function MailingEntity:mark_deleted()
  self._deleted = true
end


function MailingEntity:deleted()
  return true == self._deleted
end


function MailingEntity:data_set(args)
  if args ~= nil then
...
function MailingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MailingEntity:match_set(args)
  if args ~= nil then
...
function MailingEntity:match_get()
...
function MailingEntity:stream(action, args, callopts)
...
function MailingEntity:_run_op(ctx, post_done)
  local utility = self._utility
