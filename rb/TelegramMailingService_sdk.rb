...
require_relative 'TelegramMailingService_types'
...
class TelegramMailingServiceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TelegramMailingServiceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TelegramMailingServiceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TelegramMailingServiceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TelegramMailingServiceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TelegramMailingServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TelegramMailingServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TelegramMailingServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TelegramMailingServiceError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue TelegramMailingServiceError => err
    ...
    if fetched.is_a?(Hash)
      status = TelegramMailingServiceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TelegramMailingServiceError.new(
        "graphql_error", "TelegramMailingServiceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TelegramMailingServiceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
