...
```ruby
result, err = client.Mailing.create({
  "recipient" => # `$ARRAY`,
})
```
...
```ruby
results, err = client.Mailing.list(nil)
```
...
```ruby
result, err = client.Mailing.load({ "id" => "mailing_id" })
```
...
```ruby
result, err = client.Mailing.remove({ "id" => "mailing_id" })
```
