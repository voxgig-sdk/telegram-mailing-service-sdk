...
class TelegramMailingServiceSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TelegramMailingServiceHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TelegramMailingServiceHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TelegramMailingServiceFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TelegramMailingServiceFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TelegramMailingServiceUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TelegramMailingServiceHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TelegramMailingServiceHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TelegramMailingServiceHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TelegramMailingServiceError($op . "_allow",
                "TelegramMailingServiceSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TelegramMailingServiceHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TelegramMailingServiceError("graphql_error",
                "TelegramMailingServiceSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Mailing($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_mailing === null) {
                $this->_mailing = new MailingEntity($this, null);
            }
            return $this->_mailing;
        }
        return new MailingEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new TelegramMailingServiceSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
