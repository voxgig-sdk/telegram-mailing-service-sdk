...
// TelegramMailingService SDK feature test
//
...
class FtCtrl extends TelegramMailingServiceControl
{
...
class FtClient
{
    ...
    private TelegramMailingServiceUtility $utility;

    public function __construct(TelegramMailingServiceUtility $utility, array $options)
    {
    ...
    public function get_utility(): TelegramMailingServiceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TelegramMailingServiceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TelegramMailingServiceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TelegramMailingServiceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TelegramMailingServiceConfig::make_feature($name);
    ...
    private static function buildUrl(TelegramMailingServiceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TelegramMailingServiceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TelegramMailingServiceResponse($fetched);
        }
    ...
    private function fail(TelegramMailingServiceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TelegramMailingServiceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
