...
// TelegramMailingService SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TelegramMailingServiceTestFeature extends TelegramMailingServiceBaseFeature
{
    ...
    public function init(TelegramMailingServiceContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TelegramMailingServiceContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TelegramMailingServiceParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TelegramMailingServiceContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TelegramMailingServiceContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TelegramMailingServiceParam::call($ctx, $k);
