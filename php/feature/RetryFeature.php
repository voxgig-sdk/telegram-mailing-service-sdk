...
// TelegramMailingService SDK retry feature
...
class TelegramMailingServiceRetryFeature extends TelegramMailingServiceBaseFeature
{
    ...
    public function init(TelegramMailingServiceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TelegramMailingServiceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TelegramMailingServiceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TelegramMailingServiceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
