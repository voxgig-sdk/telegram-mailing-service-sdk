...
// TelegramMailingService SDK utility: result_basic

class TelegramMailingServiceResultBasic
{
    public static function call(TelegramMailingServiceContext $ctx): ?TelegramMailingServiceResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof TelegramMailingServiceError) ? $result->err->msg : (string)$result->err;
