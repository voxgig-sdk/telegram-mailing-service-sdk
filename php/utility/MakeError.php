...
class TelegramMailingServiceMakeError
{
    public static function call(?TelegramMailingServiceContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TelegramMailingServiceContext([], null);
        }
        $op = $ctx->op ?? new TelegramMailingServiceOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TelegramMailingServiceResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TelegramMailingServiceError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TelegramMailingServiceError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof TelegramMailingServiceError) {
