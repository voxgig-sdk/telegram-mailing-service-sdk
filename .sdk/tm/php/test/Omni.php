...
//    only - a typed TelegramMailingServiceContext there would make every ctx
...
final class TelegramMailingServiceOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TelegramMailingServiceContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TelegramMailingServiceSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TelegramMailingServiceResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TelegramMailingServiceError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TelegramMailingServiceResponse($clean['response']);
    ...
    public static function err_from_map($m): ?TelegramMailingServiceError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TelegramMailingServiceError(is_string($code) ? $code : '', $msg);
    }
