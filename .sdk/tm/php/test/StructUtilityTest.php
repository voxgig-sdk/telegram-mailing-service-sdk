...
// TelegramMailingService SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TelegramMailingServiceOmni::makeStructRunner(
    ...
    {
        return TelegramMailingServiceOmni::objmodel(TelegramMailingServiceOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return TelegramMailingServiceOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return TelegramMailingServiceOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TelegramMailingServiceOmni::structfix($now, Struct::undef()))
                        !== json_encode(TelegramMailingServiceOmni::structfix($val, Struct::undef()))) {
