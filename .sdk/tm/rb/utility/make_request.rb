...
module TelegramMailingServiceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TelegramMailingServiceResponse.new({})
    ...
    if fetch_err
      ...
      response = TelegramMailingServiceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TelegramMailingServiceResponse.new(fetched)
    else
