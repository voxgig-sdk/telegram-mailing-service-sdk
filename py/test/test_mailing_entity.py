...
class TestMailingEntity:
    ...
    def test_should_stream(self):
        ...
        base = TelegramMailingServiceSDK.test(seed, None)
        seen = list(base.Mailing(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from telegrammailingservice_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = TelegramMailingServiceSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Mailing(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TELEGRAM_MAILING_SERVICE_TEST_MAILING_ENTID JSON to run live")
        client = setup["client"]
        ...
        mailing_ref01_data = helpers.to_map(runner.entity_data(mailing_ref01_ent.create(mailing_ref01_data, None)))
        ...
        mailing_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(mailing_ref01_data_dt0_loaded))
...
def _mailing_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("TELEGRAM_MAILING_SERVICE_TEST_MAILING_ENTID"))
    ...
    _live = env.get("TELEGRAM_MAILING_SERVICE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("TELEGRAM_MAILING_SERVICE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
