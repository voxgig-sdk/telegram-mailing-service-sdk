# TelegramMailingService SDK test runner
...
class TelegramMailingServiceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TelegramMailingServiceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TelegramMailingServiceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TelegramMailingServiceTestRunner.getenv("TELEGRAM_MAILING_SERVICE_TEST_LIVE")
        override = TelegramMailingServiceTestRunner.getenv("TELEGRAM_MAILING_SERVICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TelegramMailingServiceTestRunner.getenv(key)
        ...
        explain = TelegramMailingServiceTestRunner.getenv("TELEGRAM_MAILING_SERVICE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TelegramMailingServiceTestRunner._test_control is not None:
            return TelegramMailingServiceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TelegramMailingServiceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TelegramMailingServiceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TelegramMailingServiceTestRunner.load_test_control()
...
def load_env_local():
    TelegramMailingServiceTestRunner.load_env_local()


def env_override(m):
    return TelegramMailingServiceTestRunner.env_override(m)


def entity_data(v):
    return TelegramMailingServiceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TelegramMailingServiceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TelegramMailingServiceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TelegramMailingServiceTestRunner.load_test_control()


def live_client_options():
    return TelegramMailingServiceTestRunner.live_client_options()


def live_delay_ms():
    return TelegramMailingServiceTestRunner.live_delay_ms()
