...
The Python SDK for the TelegramMailingService API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Mailing(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Mailing(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
```python
# Create
created, _ = client.Mailing(None).create({"name": "Example"}, None)

# Remove
client.Mailing(None).remove({"id": created["id"]}, None)
```
...
result, err = client.TelegramMailingService(None).load(
    {"id": "test01"}, None
)
